...
/***/ "./app/javascript/global/index.js":
/*!****************************************!*\
  !*** ./app/javascript/global/index.js ***!
  \****************************************/
...
__webpack_require__(/*! global */ "./app/javascript/global/index.js");

console.log("Yeah");

function updateNotifications() {
  $.get('/notifications', function (data, textStatus, jqXHR) {
...
window.onload = function defineCalendar() {
  var myCalender = new CalendarPicker('#calendar', {
    // If max < min or min > max then the only available day will be today.
    min: new Date(),
    max: new Date(new Date().getFullYear() + 4, 10) // NOTE: new Date(nextYear, 10) is "Sun Nov 01 nextYear"

  });
  var updatedValue = document.querySelector("#calendar-attached"); // Set the current date on load

  var date = Date.now();
  var day = fill_zeros(date.getDate(), 2);
  var month = fill_zeros(date.getMonth() + 1, 2);
  var year = fill_zeros(date.getFullYear(), 4);
  updatedValue.setAttribute("value", year + "-" + month + "-" + day); // Add callback
  ...
  myCalender.onValueChange(function (currentValue) {
    ...
    console.log("The current value of the calendar is: ".concat(currentValue));
  });
};

var CalendarPicker = function CalendarPicker(_element, options) {
  var _this = this;
  ...
  this._formatDateToInit = function (date) {
    if (!date) return;
    date.setHours(0, 0, 0);
  };

  this._insertCalendarIntoWrapper = function () {
    _this.calendarWrapper.appendChild(_this.calendarElement);
    /**
    ...
    _this.calendarGrid.addEventListener('click', handleSelectedElement, false);

    _this.calendarGrid.addEventListener('keydown', function (keyEvent) {
      if (keyEvent.key !== 'Enter') return;
      handleSelectedElement(keyEvent);
    }, false);
  };

  this._insertHeaderIntoCalendarWrapper = function () {
    _this.calendarHeaderTitle.textContent = "".concat(_this.listOfAllMonthsAsText[_this.month], " - ").concat(_this.year);

    _this.calendarHeader.appendChild(_this.calendarHeaderTitle);

    _this.calendarWrapper.appendChild(_this.calendarHeader);
  };

  this._insertCalendarGridDaysHeader = function () {
    _this.listOfAllDaysAsText.forEach(function (day) {
      var dayElement = document.createElement('span');
      dayElement.textContent = day;

      _this.calendarGridDays.appendChild(dayElement);
    });

    _this.calendarElement.appendChild(_this.calendarGridDays);
  };

  this._insertNavigationButtons = function () {
    ...
    _this.previousMonthArrow.innerHTML = arrowSvg;
    _this.nextMonthArrow.innerHTML = arrowSvg;

    _this.previousMonthArrow.setAttribute('aria-label', 'Go to previous month');

    _this.nextMonthArrow.setAttribute('aria-label', 'Go to next month');

    _this.navigationWrapper.appendChild(_this.previousMonthArrow);

    _this.navigationWrapper.appendChild(_this.nextMonthArrow);

    _this.navigationWrapper.addEventListener('click', function (clickEvent) {
      if (clickEvent.target.closest("#".concat(_this.previousMonthArrow.id))) {
        if (_this.month === 0) {
          _this.month = 11;
          _this.year -= 1;
        } else {
      ...
      if (clickEvent.target.closest("#".concat(_this.nextMonthArrow.id))) {
        if (_this.month === 11) {
          _this.month = 0;
          _this.year += 1;
        } else {
    ...
    _this.calendarElement.appendChild(_this.navigationWrapper);
  };

  this._insertDaysIntoGrid = function () {
    _this.calendarGrid.innerHTML = '';

    var arrayOfDays = _this._getDaysInMonth(_this.month, _this.year);
    ...
    arrayOfDays.forEach(function (date) {
      var dateElement = document.createElement(date ? _this.calendarDayElementType : 'span');

      var _date$toString$split = date.toString().split(' '),
          _date$toString$split2 = _slicedToArray(_date$toString$split, 4),
          Day = _date$toString$split2[0],
          Month = _date$toString$split2[1],
          Date = _date$toString$split2[2],
          Year = _date$toString$split2[3];

      var dateIsTheCurrentValue = _this.value.toString() === date.toString();
      if (dateIsTheCurrentValue) _this.activeDateElement = dateElement;

      var dateIsBetweenAllowedRange = (_this.min || _this.max) && date.toString() !== _this.today.toString() && (date < _this.min || date > _this.max);
      ...
      _this.calendarGrid.appendChild(dateElement);
    });

    _this.calendarElement.appendChild(_this.calendarGrid);

    _this.activeDateElement.classList.add('selected');
  };

  this._updateCalendar = function () {
    _this.date = new Date(_this.year, _this.month);

    var _this$date$toString$s = _this.date.toString().split(' ');

    var _this$date$toString$s2 = _slicedToArray(_this$date$toString$s, 4);

    _this.dayAsText = _this$date$toString$s2[0];
    _this.monthAsText = _this$date$toString$s2[1];
    _this.dateAsText = _this$date$toString$s2[2];
    _this.yearAsText = _this$date$toString$s2[3];
    ...
    window.requestAnimationFrame(function () {
      ...
      _this._insertDaysIntoGrid();
    });
  };

  this.onValueChange = function (callback) {
    if (_this.callback) return _this.callback(_this.value);
  ...
  this.userElement = document.querySelector(_element); // Destructuring current date as readable text.

  var _this$date$toString$s3 = this.date.toString().split(' ');

  var _this$date$toString$s4 = _slicedToArray(_this$date$toString$s3, 4);

  this.dayAsText = _this$date$toString$s4[0];
  this.monthAsText = _this$date$toString$s4[1];
  this.dateAsText = _this$date$toString$s4[2];
  this.yearAsText = _this$date$toString$s4[3];
  ...
  this.listOfAllDaysAsText = ['Dilluns', 'Dimarts', 'Dimecres', 'Dijous', 'Divendres', 'Dissabte', 'Diumenge']; // Hard-coded list of all months.

  this.listOfAllMonthsAsText = ['Gener', 'Febrer', 'Març', 'Abril', 'Maig', 'Juny', 'Juliol', 'Agost', 'Septembre', 'Octubre', 'Novembre', 'Desembre']; // Creating the calendar
...
/******/ });
//# sourceMappingURL=application-cf4c7a35fac49aa282d1.js.map